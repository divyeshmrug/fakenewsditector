...
export const saveToSQLite = (id: string, data: ChatData) => {
    ...
    INSERT OR REPLACE INTO chats (id, text, label, score, reason, factCheck)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
    stmt.run(id, data.text, data.label, data.score, data.reason, JSON.stringify(data.factCheck));
};
